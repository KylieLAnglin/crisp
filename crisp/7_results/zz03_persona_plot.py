...
# ------------------ FILTER FOR persona + BASELINE, ZERO-SHOT ------------------
df = df[(df["technique"].isin(["persona", "baseline"])) & (~df["few"])]
...
for _, row in df_wide.iterrows():
    ...
    if pd.isna(row["f1_baseline"]) or pd.isna(row["f1_persona"]):
        continue
    ...
    persona = row["f1_persona"]
    color = row["color"]

    ax.plot([x, x], [base, persona], color=color, linewidth=1.0)
    ax.annotate(
        "",
        xy=(x, persona),
        xytext=(x, base),
    ...
    ax.scatter(x, persona, facecolors=color, edgecolors=color, s=60, zorder=3)
...
# ------------------ STYLING ------------------
ax.set_title(
    "Persona-Induced F1 Score Changes by Prompt and Model (Dev Set)", fontsize=14
)
...
plt.tight_layout()
plt.savefig(
    os.path.join(start.RESULTS_DIR, "persona_arrowplot_dev.png"),
    dpi=300,
    bbox_inches="tight",
)
plt.show()
