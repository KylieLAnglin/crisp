...
negative_examples = pd.read_excel(
    start.DATA_DIR + "temp/mm_fewshot_example_performance_negative.xlsx"
)
...
for formatted_prompt in list_of_prompts:
    ...
    # drop missing
    limited = (~pd.isna(test_df.human_code.values)) & (~pd.isna(classifications))
    classifications_series = pd.Series(classifications, index=test_df.index)

    # accuracy
    accuracy = accuracy_score(
        test_df[limited].human_code,
        classifications_series[limited],
    )
...
example_performance_df.to_excel(
    start.DATA_DIR + "temp/mm_fewshot_num_example_performance.xlsx"
)
